...
overall_accuracy_binary = (total_tp + total_tn) / (total_tp + total_tn + total_fp + total_fn) if (total_tp + total_tn + total_fp + total_fn) > 0 else 0 # Renamed to avoid conflict
...
try:
    classification_stats_path = os.path.join(results_dir, 'classification_stats_subfamily.txt')
